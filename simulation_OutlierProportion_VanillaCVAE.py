...
def main(taskid):
    ...
    hyperparameter_config = {
        ...
        'dataset_name': ['BRAIX'],
        'model': ['VanillaCVAE'], #VanillaCVAE
        'simulated_outlier_type': ['outlier_exposure'], #'artificial', 'true', 'data_augmentation', 'outlier_exposure'
        'discovered_true_outlier_ratio': ['partial', 'all'],
        'outlier_prop': [0.0], #0.001, 0.005, 0.01, 0.02, 0.05, 0.1
        'resize_height': [512], #512
        'resize_width': [256],
        'image_channels': [1],
    ...
    os.system("python main.py --simulation --use_crop --taskid %s "
              "--dataset_name %s --model %s --simulated_outlier_type %s --discovered_true_outlier_ratio %s "
              "--outlier_prop %s --resize_height %s --resize_width %s --image_channels %s "
              ...
              % (taskid, temp['dataset_name'], temp['model'], temp['simulated_outlier_type'], temp['discovered_true_outlier_ratio'], temp['outlier_prop'],
